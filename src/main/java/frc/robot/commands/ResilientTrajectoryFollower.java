...
public class ResilientTrajectoryFollower extends Command {
    ...
    /**
     * When toggled from false to true, the robot's pose is instantly displaced perpendicular to its
    ...
    /** Event bindings: event name to Command to schedule when the event fires. */
    ...
    /** Debouncer for the TRACKING to RECOVERING transition. */
    ...
    public void execute() {
        ...
        if (nudgeNow && !previousNudgeState) {
            ...
            // Offset 90 degrees to the left of the robot's current heading.
